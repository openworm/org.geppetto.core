...
GEPPETTO.ControlPanel.setControlsConfig({ "VisualCapability": {}, "Common": { "plot": { "id": "plot", "actions": ["$instance$.getVariables()[0].getValue().resolve(function(){ var instancePath = ''; for (var i = 0; i < GEPPETTO.ModelFactory.allPaths.length; i++) { if (GEPPETTO.ModelFactory.allPaths[i].type == $instance$.getPath()) { instancePath = GEPPETTO.ModelFactory.allPaths[i].path + '.recording'; break; } } G.addWidget(0).setSize(273.8,556.8).plotXYData(Instances.getInstance(instancePath), nwbSample.time).setPosition(130,35).setName(instancePath); }); "], "icon": "fa-area-chart", "label": "Plot", "tooltip": "Plot Sweep" } } });
